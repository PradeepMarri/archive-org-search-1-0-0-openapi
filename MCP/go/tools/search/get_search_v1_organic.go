...
func Get_search_v1_organicHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/search/v1/organic%s", cfg.BaseURL, queryString)
		...
		var result models.OrganicResult
...
func CreateGet_search_v1_organicTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_search_v1_organic",
		mcp.WithDescription("Return relevance-based results from search queries
"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_search_v1_organicHandler(cfg),
	}
}
